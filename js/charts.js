...
function drawInvestmentProjectionLegend(ctx, canvas, width, height, legendItems) {
  ...
  // Match drawLegend() geometry exactly
  const boxPaddingY = 3;
  const boxRadius = 12;
  const boxX = 24;
  ...
  const contentWidth = boxWidth - panelPaddingX * 2;
  ...
  // Match generic legend vertical feel
  const rowHeight = 34;
  const rowGap = 0;
  const contentHeight = rows.length * rowHeight + (rows.length - 1) * rowGap;
  ...
  rows.forEach((row) => {
    row.forEach((item, colIndex) => {
      ...
      const x = boxX + panelPaddingX + colIndex * (columnWidth + columnGap);
